...
//		  setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
        ...
        if (wifiStatus == null)
            wifiStatus = new WifiStatus(getSystemService(Context.CONNECTIVITY_SERVICE));
        ...
        //Initial checks
        if (wifiCon == null)
            wifiCon = new WifiController(this, getSystemService(Context.CONNECTIVITY_SERVICE));
                ...
                if (db.numberOfRowsEAP() > 0) {
                    if (profiles.size() > 0 && profiles != null)
                        if (profiles.get(0).hasHelpdeskInfo()) {
                            ...
                            String helpPhone = "";
                            String helpURL = "";
                            if (profiles.get(0).getHelpdeskURL() != null) {
                                helpURL = profiles.get(0).getHelpdeskURL().toString();
                            }
                            if (profiles.get(0).getHelpdeskPhoneNumber() != null) {
                                helpPhone = profiles.get(0).getHelpdeskPhoneNumber();
                            }
                            if (profiles.get(0).getSupportEmails() != null) {
                                helpEmail = profiles.get(0).getSupportEmails();
                            }
                            if (profiles.get(0).getDisplayName() != null) {
                                displayName = profiles.get(0).getDisplayName();
                            }
                            String message = getString(R.string.support_message2, displayName);
                            message += " ";
                            message += getString(R.string.support_message3) + "\n";
                            if (helpEmail.length() > 0)
                                message += getString(R.string.support_email) + helpEmail + "\n";
                            if (helpPhone.length() > 0)
                                message += getString(R.string.support_phone) + helpPhone + "\n";
                            if (helpURL.length() > 0)
                                message += getString(R.string.support_web) + helpURL;
