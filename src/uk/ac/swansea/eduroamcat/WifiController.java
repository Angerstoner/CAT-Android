        ...
        if (wifi.isWifiEnabled()) {
            ...
            if (scan.size() > 0) {
                ...
                for (ScanResult scan_result : scan) {
                    ssid_array[count] = scan_result.SSID + "[" + scan_result.capabilities + "]";
                    eduroamCAT.debug("Settings" + scan_result.capabilities);
                    count++;
                }
                return ssid_array;
            } else {
        ...
        if (wifi.isWifiEnabled()) {
            ...
            for (WifiConfiguration currentConfig : currentConfigs) {
                if (currentConfig.SSID.contains(sSID)) {
                    ...
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PEAP)
                        eduroamCAT.debug("Set to PEAP");
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                        eduroamCAT.debug("Set to PWD");
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TLS)
                        eduroamCAT.debug("Set to TLS");
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TTLS)
                        eduroamCAT.debug("Set to TTLS");
                    eduroamCAT.debug("PHASE2:" + currentConfig.enterpriseConfig.getPhase2Method());
                    if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.MSCHAPV2)
                        eduroamCAT.debug("Set to MSCHAPv2 OK");
                    if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.GTC)
                        eduroamCAT.debug("Set to GTC OK");
                    if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.PAP)
        ...
        if (wifi.isWifiEnabled()) {
            ...
            for (WifiConfiguration currentConfig : currentConfigs) {
                if (currentConfig.SSID.equals("\"eduroam\"")) {
                    ...
                    if (currentConfig.allowedGroupCiphers.toString().contains(","))
                        message += " with mixed mode";
                    else if (currentConfig.allowedGroupCiphers.toString().contains("2"))
                        message += " with CCMP";
                    else if (currentConfig.allowedGroupCiphers.toString().contains("2"))
                        message += " with TKIP";
                    message += "</font> <br/>";
                    ...
                    if (currentConfig.enterpriseConfig.getAnonymousIdentity().length() > 0)
                        message += "Anon ID:<font color=\"green\">" + currentConfig.enterpriseConfig.getAnonymousIdentity() + "</font>";
                    ...
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PEAP)
                        message += "<font color=\"green\">PEAP</font> with ";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                        message += "<font color=\"green\">PWD</font> ";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TLS)
                        message += "<font color=\"green\">TLS</font>";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TTLS)
                        message += "<font color=\"green\">TTLS</font> with ";
                    else message += "<font color=\"red\">EAP Method error</font>";
                    if (currentConfig.enterpriseConfig.getEapMethod() != Eap.TLS) {
                        if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.MSCHAPV2)
                            message += "Phase2:<font color=\"green\">MSCHAPv2</font>";
                        else if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.GTC)
                            message += "Phase2:<font color=\"green\">GTC</font>";
                        else if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.PAP)
                            message += "Phase2:<font color=\"green\">PAP</font>";
                        else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                            message += "";
                        else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TLS)
                            message += "";
                    ...
                    if (nullpoint == -1) {
                        ...
                        if (start > 0 && finish > 0)
                            message += "<br/><font color=\"green\">CA Certificate OK";
                    } else {
                        if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                            message += "<font color=\"black\">CA Certificate unrequired</font>";
                        else message += "<br/><font color=\"red\">No CA certificate found</font>";
                    }
                    if (currentConfig.enterpriseConfig.getSubjectMatch() != null) {
                        if (currentConfig.enterpriseConfig.getSubjectMatch().length() > 0)
                            message += "<br/>Server Subject Match=<font color=\"green\">" + currentConfig.enterpriseConfig.getSubjectMatch() + "</font>";
                        else
                            message += "<br/><font color=\"red\">Server Subject Match missing</font>";
                    } else
                        message += "<br/><font color=\"red\">Server Subject Match missing</font>";
                }
            }
                ...
                for (WifiConfiguration currentConfig : currentConfigs) {
                    if (currentConfig.SSID.equals("\"eduroam\"")) {
                        if (currentConfig.enterpriseConfig.getAnonymousIdentity().length() > 0)
                            ...
                            message += "<font color=\"black\">Anon ID unrequired</font>";
        ...
        if (wifi.isWifiEnabled()) {
            ...
            for (WifiConfiguration currentConfig : currentConfigs) {
                if (currentConfig.SSID.equals("\"eduroam\"")) {
                    message += "EAP Method=";
                    if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PEAP)
                        message += "<font color=\"black\">PEAP</font> with ";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                        message += "<font color=\"black\">PWD</font>";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TLS)
                        message += "<font color=\"black\">TLS</font>";
                    else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TTLS)
                        message += "<font color=\"black\">TTLS</font> with ";
                    else message += "<font color=\"#000001\">EAP Method error</font>";
                    if (currentConfig.enterpriseConfig.getEapMethod() != Eap.TLS) {
                        if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.MSCHAPV2)
                            message += "Phase2:<font color=\"black\">MSCHAPv2</font>";
                        else if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.GTC)
                            message += "Phase2:<font color=\"black\">GTC</font>";
                        else if (currentConfig.enterpriseConfig.getPhase2Method() == Phase2.PAP)
                            message += "Phase2:<font color=\"black\">PAP</font>";
                        else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.PWD)
                            message += "";
                        else if (currentConfig.enterpriseConfig.getEapMethod() == Eap.TLS)
                            message += "";
        ...
        if (wifi.isWifiEnabled()) {
            ...
            for (WifiConfiguration currentConfig : currentConfigs) {
                if (currentConfig.SSID.equals("\"eduroam\"")) {
                    if (currentConfig.enterpriseConfig.getSubjectMatch() != null) {
                        if (currentConfig.enterpriseConfig.getSubjectMatch().length() > 0)
                            message += "Server Subject Match=<font color=\"black\">" + currentConfig.enterpriseConfig.getSubjectMatch() + "</font>";
                        else
                            message += "<font color=\"#000001\">Server Subject Match missing</font>";
